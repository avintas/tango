...
export default function PromptGeneratorPage() {
  ...
  const fetchVariables = async () => {
    ...
    try {
      const response = await fetch('/api/prompt-variables?type=hugs', {
        headers: {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch('/api/gemini/hugs-generate', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="space-y-6">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-900">HUGs Generator</h1>
        <p className="mt-2 text-gray-600 dark:text-gray-400">
          Generate heartwarming, uplifting hockey content (Hockey&apos;s
          Ultimate Gestures)
        </p>
      </div>
