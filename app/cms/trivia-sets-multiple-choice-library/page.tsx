...
export default function TriviaSetsMultipleChoiceLibraryPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/trivia-sets/multiple-choice?status=${statusFilter}`,
      );
  ...
  const handleBulkPublish = async () => {
    ...
    try {
      const promises = Array.from(selectedIds).map((id) =>
        fetch(`/api/trivia-sets/multiple-choice/${id}`, {
          method: "PATCH",
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/trivia-sets/multiple-choice/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          <Heading level={1}>📋 Multiple Choice Trivia Sets</Heading>
          <p className="mt-2 text-gray-600">
            Curated Multiple Choice trivia sets ready to play
          </p>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
            <p className="text-gray-500">
              No {statusFilter} Multiple Choice trivia sets found.
            </p>
            <Link
