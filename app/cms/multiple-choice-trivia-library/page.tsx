...
export default function MultipleChoiceTriviaLibraryPage() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const response = await fetch(`/api/multiple-choice-trivia?stats=true`);
  ...
  const fetchContent = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/multiple-choice-trivia?status=${statusFilter}&limit=${limit}&offset=${offset}`,
      );
      const result = await response.json();
              ...
              {items.length === 0 ? (
                <div className="p-8 text-center bg-white border border-gray-200 rounded-lg">
                  <p className="text-gray-500">
                    No {getStatusLabel(statusFilter).toLowerCase()}{" "}
                    multiple-choice trivia questions found.
                  </p>
                </div>
              ) : (
                items.map((item) => (
