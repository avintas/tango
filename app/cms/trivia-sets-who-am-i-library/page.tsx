...
export default function TriviaSetsWhoAmILibraryPage() {
  const [items, setItems] = useState<TriviaSetWhoAmI[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/trivia-sets/who-am-i?status=${statusFilter}`,
      );
  ...
  const handleBulkPublish = async () => {
    ...
    try {
      const promises = Array.from(selectedIds).map((id) =>
        fetch(`/api/trivia-sets/who-am-i/${id}`, {
          method: "PATCH",
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/trivia-sets/who-am-i/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          <Heading level={1}>🎭 Who Am I? Trivia Sets</Heading>
          <p className="mt-2 text-gray-600">
            Curated Who Am I trivia sets ready to play
          </p>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
            <p className="text-gray-500">
              No {statusFilter} Who Am I? trivia sets found.
            </p>
            <Link
