/**
 * AI Extraction Prompts Configuration
 * File-based storage for AI prompts used in source content ingestion workflow
 *
 * Benefits:
...
export interface AIExtractionPromptConfig {
  prompt_name: string;
 ...
 */
export const METADATA_EXTRACTION_PROMPT: AIExtractionPromptConfig = {
  prompt_name: "Hockey Content Metadata Extraction",
  prompt_type: "metadata_extraction",
  prompt_content: `You are an expert at analyzing hockey-related content and extracting structured metadata.
 ...
 */
export const CONTENT_ENRICHMENT_PROMPT: AIExtractionPromptConfig = {
  prompt_name: "Hockey Content Enrichment",
  prompt_type: "content_enrichment",
  prompt_content: `You are an expert at analyzing hockey-related content and generating enriched metadata.
...
): AIExtractionPromptConfig | null {
  if (promptType === "metadata_extraction") {
    return METADATA_EXTRACTION_PROMPT.is_active
      ? METADATA_EXTRACTION_PROMPT
      : null;
  }
  if (promptType === "content_enrichment") {
    return CONTENT_ENRICHMENT_PROMPT.is_active
 ...
 * Get all prompts
 */
export function getAllPrompts(): AIExtractionPromptConfig[] {
  return [METADATA_EXTRACTION_PROMPT, CONTENT_ENRICHMENT_PROMPT];
}
