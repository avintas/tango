...
export default function TrueFalseTriviaCard({
  ...
  const handleArchive = async () => {
    ...
    setIsArchiving(true);
    try {
      const response = await fetch(`/api/true-false-trivia/${item.id}`, {
        method: "PATCH",
    ...
    <div className="p-4 rounded-lg bg-white border border-gray-300">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          {/* Question */}
          <p className="text-gray-900 mb-2 font-medium">{item.question_text}</p>
          ...
          <div className="flex items-center gap-2 flex-wrap text-xs text-gray-600">
            {/* Type */}
            <span className="text-indigo-600 font-medium">True/False</span>
